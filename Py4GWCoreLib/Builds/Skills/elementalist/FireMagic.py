...
class FireMagic:
    ...
    def Burning_Speed(self) -> BuildCoroutine:
        ...
        Skipped when the enchantment is already on the caster so the self-burning
        is not re-applied mid-duration.
        """
        ...
        if not Routines.Agents.GetNearestEnemy(Range.Nearby.value):
            return False
        if Routines.Checks.Agents.HasEffect(Player.GetAgentID(), burning_speed_id):
            return False
